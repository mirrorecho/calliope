        ...
        if self.pitch_ranges is not None:
            for c in range(len(self.data.columns)):
                for r in range(len(self.data)):
                    pitch_range = self.pitch_ranges.iat[c % len(self.pitch_ranges.columns)].iat[r % len(self.pitch_ranges)]
                    # TO DO: consider... always move to new random choice? Or only move if pitch not already in range?
                    self.data[c][r] = random.choice(pitch_range.voice_pitch_class(self.data[c][r])).number
        else:
